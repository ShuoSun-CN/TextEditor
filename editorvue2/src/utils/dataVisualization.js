...
class dataVisual {
    ...
    getOptions() {
        return [
            {value:"default",text:"数据可视化",styleForRenderMenuList: { 'font-size': '13px', 'font-weight': 'bold' }},
            {value: "bar", text: '柱状图'},
            {value: "line", text: '折线图'},
            {value: "pie", text: '饼状图'},
        ];
    ...
    async exec(editor, value) {
        ...
        try {
            ...
            if (data.code === 0 && data.polishedText) {
                //获取json文件
                console.log("data.polishedText:", data.polishedText);
                //生成图片
                const imageBase64 = await this.generateChartImage(data.polishedText);
                //将图片上传到服务器
    ...
    //生成图片
    async generateChartImage(jsonString) {
        let option;
        try {
    ...
    async uploadImage(base64Image) {
        ...
        formData.append('file', this.base64ToBlob(base64Image), 'chart.png');
    ...
    showPopup(imagUrl) {
        ...
        resultImg.src = imagUrl;


        const insertTextButton = document.createElement('button');
        insertTextButton.textContent = '插入图片';
