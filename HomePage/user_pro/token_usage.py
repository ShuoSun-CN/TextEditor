...
@csrf_exempt
def get_consume_token_usage(req):
    try:
        user_id=verify_session_uid(req)
        if user_id is None:
            return JsonResponse({
        ...
        use_info=TokenUse.objects.filter(user_id=user_id,consume_tokens__lt=0).order_by('-consume_time')
        response=[]
        for i in use_info:
            response.append(i.get_dict())
        return JsonResponse({
...
@csrf_exempt
def get_obtain_token_usage(req):
    try:
        user_id=verify_session_uid(req)
        if user_id is None:
            return JsonResponse({
        ...
        use_info=TokenUse.objects.filter(user_id=user_id,consume_tokens__gt=0).order_by('-consume_time')
        response=[]
        for i in use_info:
            response.append(i.get_dict())
        return JsonResponse({
